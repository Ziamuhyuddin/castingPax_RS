...
import {Colors, Fonts, hp, IS_IPHONE_X, isTab, Strings, wp} from '../../../constants';

interface PerformorStatus {
  rateText?:string
  costumeQuantity?:string;
  onPress?: any;
...
const PerformorStatus: FC<PerformorStatus> = (props: PerformorStatus) => {
  ...
  const _renderShowInfo = (topText: string) => {
    ...
    return (
      <TouchableOpacity onPress={()=>onBlogPress(topText)}
        ...
        <Text style={styles.bottomText}>{rateText}</Text>
            <Image style={styles.chevronDown}
          ...
          topText==='Rental'&&
              <View style={styles.RentalCon} >
                ...
                <Switch
                    ...
                    trackColor={{ false: Colors.GREY.opacity26, true: Colors.GREEN.default }}
...
const styles = StyleSheet.create({
  ...
  switchCon:{
     transform: [{ scaleX:isTab?.9:Platform.OS==='ios'?.5: .6 }, { scaleY: isTab?.9:Platform.OS==='ios'?.5: .6 }]
  },
  RentalCon:{
    marginTop:isTab?hp(0.5):Platform.OS==='ios'?hp(-0.5):hp(-1.3),
  },
  costumeCon:{
    marginTop:Platform.OS==='ios'?hp(0.5):0,
  },
